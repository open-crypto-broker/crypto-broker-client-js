...
export const HealthListResponse: MessageFns<HealthListResponse> = {
  encode(message: HealthListResponse, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    globalThis.Object.entries(message.statuses).forEach(([key, value]: [string, HealthCheckResponse]) => {
    ...
    return {
      statuses: isObject(object.statuses)
        ? (globalThis.Object.entries(object.statuses) as [string, any][]).reduce(
          (acc: { [key: string]: HealthCheckResponse }, [key, value]: [string, any]) => {
  ...
  toJSON(message: HealthListResponse): unknown {
    const obj: any = {};
    if (message.statuses) {
      const entries = globalThis.Object.entries(message.statuses) as [string, HealthCheckResponse][];
      if (entries.length > 0) {
        obj.statuses = {};
        entries.forEach(([k, v]) => {
  ...
  fromPartial<I extends Exact<DeepPartial<HealthListResponse>, I>>(object: I): HealthListResponse {
    const message = createBaseHealthListResponse();
    message.statuses = (globalThis.Object.entries(object.statuses ?? {}) as [string, HealthCheckResponse][]).reduce(
      (acc: { [key: string]: HealthCheckResponse }, [key, value]: [string, HealthCheckResponse]) => {
        if (value !== undefined) {
