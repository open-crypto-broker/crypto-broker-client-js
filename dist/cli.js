...
async function execute(cryptoLib) {
    ...
    if (command === 'hash') {
        ...
        if (!data || !profile) {
            throw new Error('Hash command requires 2 arguments: <data> and <profile>');
        }
        ...
        if (!profile || !csrPath || !signingKeyPath || !caCertPath) {
            throw new Error('Usage: cert <profile> <csrPath> <signingKeyPath> <caCertPath>');
        }
        ...
        throw new Error(`Unsupported command '${command}, only available commands are hash and sign'`);
    }
}
async function main() {
